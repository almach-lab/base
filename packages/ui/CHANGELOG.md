# @almach/ui

## 2.0.0

### Major Changes

- 28b2346: Rework `@almach/ui` to React Aria foundations and remove Base UI primitives.

  ### Breaking changes

  - Removed `@base-ui/react` from `@almach/ui`.
  - Reworked interactive components to React Aria / React Aria Components behavior and state models.
  - Unified compound component anatomy across form, overlay, and selection primitives.
  - Updated data-attribute/state handling and internal composition paths.
  - Removed legacy Radix/Base UI-specific selector assumptions.

  ### Migration notes

  - Audit custom CSS selectors and update state hooks to the new React Aria-driven attributes/classes.
  - Review trigger/content/item/value composition for overlays and selection controls.
  - Re-test keyboard and focus-visible behavior for all wrapped primitives.
  - See `packages/ui/MIGRATION.md` for detailed old-to-new API notes.

### Minor Changes

- auto release from 5ac676a..28b2346

## 1.2.1

### Patch Changes

- d091481: Fix `SwipeActions` timer typing for strict TypeScript builds and align docs component navigation with a shared registry so routes and sidebar/search stay in sync.

## 1.2.0

### Minor Changes

- auto release from 82e45c9..f3a50d9

## 1.1.2

### Patch Changes

- auto release from 3736c08..6a60426

## 1.1.1

### Patch Changes

- 0798793: feat: implement swipe components

## 1.1.0

### Minor Changes

- auto release from 82e45c9..63f48b5

## 1.0.0

### Minor Changes

- auto release from 04a13e2..b455409

### Patch Changes

- cea8524: fix: support universal
- Updated dependencies
- Updated dependencies [cea8524]
  - @almach/utils@0.2.0

## 0.1.10

### Patch Changes

- fb35a0e: fix: toast components stylings and modals

## 0.1.9

### Patch Changes

- a4da144: feat: add modal family views

## 0.1.8

### Patch Changes

- 0940bb8: fix: make the modal responsives based on max height

## 0.1.7

### Patch Changes

- a491527: fix: bun workspace deps

## 0.1.6

### Patch Changes

- de27734: feat: add currency on the text fields form

## 0.1.5

### Patch Changes

- f08efc8: improve typings

## 0.1.4

### Patch Changes

- 1f2c712: feat: implement input for currency

## 0.1.3

### Patch Changes

- 80076c1: fix: better dx and grouing the components, merge components

## 0.1.2

### Patch Changes

- 3944560: fix: improve the components (#13)
- fa4f200: fix: improve the ui components to be more clean and smooth

## 0.1.1

### Patch Changes

- acd3a57: feat: better docs with astro (#8)
- b10edfe: feat: implement the ui for better the ux, with cursor
