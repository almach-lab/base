...
function DialogRoot({ open, defaultOpen = false, onOpenChange, children }: DialogRootProps) {
...
const DialogTrigger = React.forwardRef<HTMLButtonElement, DialogTriggerProps>(({ asChild, children, onClick, ...props }, ref) => {
...
const DialogClose = React.forwardRef<HTMLButtonElement, DialogCloseProps>(({ asChild, children, onClick, ...props }, ref) => {
...
interface DialogContentProps extends React.HTMLAttributes<HTMLDivElement> { hideClose?: boolean | undefined; }
const DialogContent = React.forwardRef<HTMLDivElement, DialogContentProps>(({ className, children, hideClose, ...props }, ref) => (
...
const DialogContentInner = React.forwardRef<HTMLDivElement, DialogContentProps>(({ className, children, hideClose, ...props }, ref) => {
	const { open, setOpen } = useDialogCtx();
	const [mounted, setMounted] = React.useState(open);
	const [isVisible, setIsVisible] = React.useState(false);

	React.useEffect(() => {
		const motionMs = resolveMotionDurationMs(MOTION_VAR_OVERLAY_DURATION, DIALOG_ANIMATION_MS);
		if (open) {
			setMounted(true);
