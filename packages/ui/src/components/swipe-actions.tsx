...
function SwipeActionsRoot({
  ...
  const setDragState = React.useCallback(
    (side: SwipeSide, past: boolean, fsReady = false) => {
      setDragSide(side);
...
function SwipeActionsContent({
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drag.current.axis === "x") {
      ...
      const fsReady =
        fullSwipe && cw > 0 && Math.abs(raw) >= cw * fullSwipeThreshold;
      ...
      const fsReady =
        fullSwipe && ch > 0 && Math.abs(raw) >= ch * fullSwipeThreshold;
      setDragState(side, past, fsReady);
    }
  };
  ...
  const onPointerUp = () => {
    ...
    if (axis === "x") {
      ...
      if (fullSwipe && cw > 0) {
        if (lw > 0 && x > cw * fullSwipeThreshold) {
          triggerFullSwipe("left");
          return;
        }
        if (rw > 0 && x < -cw * fullSwipeThreshold) {
      ...
      const bh = bottomRef.current?.offsetHeight ?? 0;
      if (fullSwipe && ch > 0) {
        if (th > 0 && y > ch * fullSwipeThreshold) {
          triggerFullSwipe("top");
          return;
        }
        if (bh > 0 && y < -ch * fullSwipeThreshold) {
          triggerFullSwipe("bottom");
          return;
        }
      }
...
function SwipeActionsLeft({
  className,
  children,
  ...props
}: React.HTMLAttributes<HTMLDivElement>) {
  const { leftRef, openSide, dragSide, pastThreshold, fullSwipeReady } =
    useSwipeCtx();
...
function SwipeActionsRight({
  ...
  const { rightRef, openSide, dragSide, pastThreshold, fullSwipeReady } =
    useSwipeCtx();
...
function SwipeActionsTop({
  className,
  children,
  ...props
}: React.HTMLAttributes<HTMLDivElement>) {
  const { topRef, openSide, dragSide, pastThreshold, fullSwipeReady } =
    useSwipeCtx();
...
function SwipeActionsBottom({
  className,
  children,
  ...props
}: React.HTMLAttributes<HTMLDivElement>) {
  const { bottomRef, openSide, dragSide, pastThreshold, fullSwipeReady } =
    useSwipeCtx();
