import { Tab, TabList, TabPanel, Tabs as TabsPrimitive } from "react-aria-components";
...
const tabsTriggerVariants = cva(cn("inline-flex cursor-pointer items-center justify-center whitespace-nowrap text-sm font-medium outline-none", MOTION_INTERACTIVE), {
	variants: {
		variant: {
			pill: [
...
interface TabsListProps extends React.ComponentPropsWithoutRef<typeof TabList>, VariantProps<typeof tabsListVariants> {}

const TabsList = React.forwardRef<HTMLDivElement, TabsListProps>(({ className, variant = "pill", children, ...props }, ref) => (
	<TabsVariantCtx.Provider value={variant ?? "pill"}>
		<ScrollArea.Root className={cn(variant === "pill" && "rounded-xl")}>
			<ScrollArea.Viewport className="border-0 bg-transparent rounded-none">
				<TabList ref={ref} className={cn(tabsListVariants({ variant }), className)} {...props}>
					{children}
				</TabList>
			</ScrollArea.Viewport>
		</ScrollArea.Root>
...
const TabsTrigger = React.forwardRef<HTMLDivElement, TabsTriggerProps>(({ className, ...props }, ref) => {
...
interface TabsContentProps extends React.ComponentPropsWithoutRef<typeof TabPanel> {}

const TabsContent = React.forwardRef<HTMLDivElement, TabsContentProps>(({ className, ...props }, ref) => (
	<TabPanel
		ref={ref}
		className={cn("mt-3 outline-none", "focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-lg", className)}
		{...props}
	/>
));
