...
const selectTriggerMotionClasses =
	cn(MOTION_INTERACTIVE, "hover:bg-muted/45 active:scale-[0.995]");
...
function SelectRoot({ value, defaultValue, onValueChange, isDisabled = false, children }: SelectRootProps) {
...
const SelectValue = ({ placeholder = "Select...", className }: { placeholder?: string; className?: string }) => {
	const { value, items } = useSelectCtx();
	const label = value ? items.get(value) ?? value : placeholder;
	const isPlaceholder = !value;
	return <span className={cn("line-clamp-1", isPlaceholder && "opacity-60", className)}>{label}</span>;
};

const SelectIndicator = ({ className, children }: { className?: string; children?: React.ReactNode }) => (
	<span className={cn("flex items-center justify-center", className)}>
		{children ?? <ChevronDown className="h-4 w-4 shrink-0 opacity-50 transition-transform duration-150 ease-out" />}
	</span>
);

const SelectTrigger = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { error?: boolean }>(
...
const SelectItem = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { value: string }>(
	({ className, children, value, disabled, ...props }, ref) => {
		const { value: selectedValue, onValueChange, setOpen, registerItem, isDisabled } = useSelectCtx();
...
const SelectContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, children, ...props }, ref) => {
	...
	return createPortal(
		<div
			...
			{...props}
		>
			<div className="relative overflow-y-auto py-1 scroll-py-6" style={position ? { maxHeight: position.maxHeight } : undefined}>
				{children}
			</div>
		</div>,
		document.body,
...
const SelectLabel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
	<div ref={ref} className={cn("px-2.5 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground", className)} {...props} />
));
SelectLabel.displayName = "Select.Label";

const SelectDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => (
	<p ref={ref} className={cn("text-sm text-muted-foreground", className)} {...props} />
));
SelectDescription.displayName = "Select.Description";

function SelectSeparator({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
	return <div className={cn("-mx-1.5 my-1 h-px bg-border", className)} {...props} />;
}

const SelectGroup = ({ children }: { children?: React.ReactNode }) => <div>{children}</div>;
const SelectScrollUpButton = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => <div className={className} {...props} />;
const SelectScrollDownButton = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => <div className={className} {...props} />;
	...
	return (
		<div className="relative">
			...
			>
				...
				<ChevronsUpDown className={cn("ml-2 h-4 w-4 shrink-0 opacity-50 transition-transform duration-150 ease-out", open && "rotate-180")} />
			</button>

			{open ? (
				<div className={cn(selectPopupClasses, "absolute z-50 mt-2 min-w-full rounded-[1.25rem] p-0")}>
					<CommandPrimitive className="flex flex-col" aria-label={placeholder}>
						...
						<CommandPrimitive.List className="max-h-56 overflow-y-auto p-1" role="listbox" aria-label="Options">
							<CommandPrimitive.Empty className="py-4 text-center text-sm text-muted-foreground">{empty}</CommandPrimitive.Empty>
							...
							{options.map((opt) => (
								...
								>
									<Check className={cn("h-4 w-4 shrink-0", value === opt.value ? "opacity-100" : "opacity-0")} aria-hidden="true" />
									{opt.label}
								</CommandPrimitive.Item>
							))}
