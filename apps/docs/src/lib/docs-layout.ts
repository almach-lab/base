...
export const docsLayout = {
  pageRoot:
    "flex min-h-dvh flex-col lg:h-dvh lg:min-h-0 lg:overflow-hidden",
  pageHeader:
    "sticky top-0 z-40 shrink-0 border-b border-border/40 bg-background/90 backdrop-blur-md",
  shell:
    "flex min-h-0 flex-1 w-full lg:overflow-hidden [&>*]:min-h-0",
  sidebar:
    "hidden w-68 min-h-0 shrink-0 flex-col border-r border-border/35 lg:flex",
  main:
    "min-w-0 flex-1 scroll-smooth scroll-fade no-scrollbar overflow-y-auto px-5 py-8 lg:px-10 lg:py-9",
  content: "mx-auto w-full max-w-3xl",
  toc:
    "hidden w-56 min-h-0 shrink-0 scroll-fade no-scrollbar overflow-y-auto py-8 pl-4 pr-6 lg:block",
  header:
  ...
  headerActions:
    "flex items-center justify-end gap-1 sm:gap-1.5 lg:pr-4",
