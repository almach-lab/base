...
<p className="text-sm text-muted-foreground">Tags: {tags.join(", ")}</p>`,
				},
				{
					...
					description: "max={3} prevents adding more than 3 tags. Input hides at limit.",
					preview: (
						<TagInput
  ...
  <TagInput id="tags" placeholder="Add a skill and press Enter…" />
			...
			props={[
				...
				},
				{
					name: "max",
					type: "number",
					description: "Maximum number of tags. Input hides when limit is reached.",
				},
				{
					name: "transform",
	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			<p className="text-xs text-muted-foreground">
				Tags: <span className="font-medium text-foreground">{tags.join(", ") || "none"}</span>
			</p>
		</div>
	);
}
