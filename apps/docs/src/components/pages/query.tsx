...
const postsQuery = createQuery({
  queryKey: () => ["posts"],
  queryFn: async () => { await delay(500); return [...mockPosts]; },
  staleTime: 0,
});
...
function PostList() {
  ...
  const { data: posts, isLoading, isFetching, refetch } = useQuery(postsQuery.options());
          ...
          <Button
            ...
            onClick={() =>
              createPost.mutate(`New post #${nextId}`, {
                onSuccess: (post) =>
                  showToast({ title: `Created "${post.title}"`, variant: "success" }),
              })
            }
          >
            Add
        ...
        {posts?.map((post) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="text-[11px]">user {post.userId}</Badge>
            </div>
            <Button
              ...
              onClick={() =>
                deletePost.mutate(post.id, {
                  onSuccess: () => showToast({ title: `Deleted "${post.title}"` }),
                })
              }
            >
        ...
        <div className="rounded-lg border px-4 py-3 text-sm space-y-2">
          {[
            ...
            <div key={label} className="flex gap-3">
              <span className="w-14 text-muted-foreground shrink-0 font-mono text-xs">{label}</span>
              <span>{value}</span>
            </div>
          ))}
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 mb-12">
        ...
        <Card>
          <Card.Header>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Switch IDs — uses <code className="font-mono text-xs">keepPreviousData</code>.
              ID 99 shows an error state.
            </p>
          </Card.Header>
          <Card.Content>
            <SinglePost />
            ...
            <Tabs.Content value="api">
              <div className="mt-2 rounded-xl border p-5">
                <div className="grid gap-4 sm:grid-cols-2 text-sm">
                  {[
                    { prop: "queryKey", type: "(...args) => QueryKey", desc: "Factory for the cache key." },
                    { prop: "queryFn", type: "(...args) => Promise<T>", desc: "Fetches the data." },
                    { prop: "staleTime", type: "number (ms)", desc: "How long data is fresh." },
                    { prop: ".options(...args)", type: "queryOptions", desc: "Returns TanStack-compatible options." },
                  ].map(({ prop, type, desc }) => (
          ...
          <Tabs defaultValue="code">
            ...
            <Tabs.Content value="api">
              <div className="mt-2 rounded-xl border p-5">
                <div className="grid gap-4 sm:grid-cols-2 text-sm">
                  {[
                    { prop: "mutationFn", type: "(input) => Promise<T>", desc: "Performs the mutation." },
                    { prop: "invalidates", type: "QueryKey[]", desc: "Query keys to refetch on success." },
                    { prop: "onSuccess", type: "(data) => void", desc: "Called after success." },
                    { prop: "onError", type: "(error) => void", desc: "Called after error." },
                  ].map(({ prop, type, desc }) => (
                    <div key={prop} className="space-y-0.5">
                      <p className="font-mono text-xs font-medium text-foreground">{prop}</p>
                      <p className="font-mono text-[11px] text-muted-foreground/70">{type}</p>
