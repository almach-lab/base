...
function ShowcaseCanvas() {
  const TABS = ["Examples", "Dashboard", "Tasks", "Playground", "Authentication"] as const;
      ...
      return (
        <div className="grid gap-3 lg:grid-cols-12">
          ...
          <Card className="border-border/70 bg-background lg:col-span-3">
            <Card.Content className="space-y-3 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Team</p>
              <div className="rounded-lg border border-border/70 bg-muted/20 p-3 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Invite your team to collaborate.</p>
              </div>
          ...
          <Card className="border-border/70 bg-background lg:col-span-5">
            <Card.Content className="space-y-3 p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Settings</p>
      ...
      return (
        <div className="grid gap-3 lg:grid-cols-12">
          {[
            ...
            <Card key={item.label} className="border-border/70 bg-background lg:col-span-3">
          ...
          <Card className="border-border/70 bg-background lg:col-span-8">
            <Card.Content className="p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Revenue trend</p>
          ...
          <Card className="border-border/70 bg-background lg:col-span-4">
            <Card.Content className="space-y-2 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Top channels</p>
              {[
                ...
                <div key={item.name} className="flex items-center justify-between rounded-md border border-border/70 px-2.5 py-2 text-sm">
                  <span>{item.name}</span>
      ...
      return (
        <div className="grid gap-3 lg:grid-cols-12">
          <Card className="border-border/70 bg-background lg:col-span-7">
            <Card.Content className="space-y-3 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Sprint board</p>
              {[
                ...
                <label key={task} className="flex items-center gap-2 rounded-md border border-border/70 px-3 py-2 text-sm">
          ...
          <Card className="border-border/70 bg-background lg:col-span-5">
            <Card.Content className="space-y-3 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Create task</p>
    ...
    if (activeTab === "Playground") {
      return (
        <div className="grid gap-3 lg:grid-cols-12">
          <Card className="border-border/70 bg-background lg:col-span-8">
            <Card.Content className="space-y-3 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Prompt playground</p>
              <Input placeholder="Ask, search, or make anything..." />
              <div className="rounded-lg border border-border/70 bg-muted/20 p-3 text-sm text-muted-foreground">
                Suggestion: \"Generate settings panel with switches and badges.\"
              </div>
          ...
          <Card className="border-border/70 bg-background lg:col-span-4">
            <Card.Content className="space-y-2 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Tokens</p>
              {[
                { label: "Input", value: "1,482" },
                { label: "Output", value: "604" },
                { label: "Latency", value: "1.2s" },
              ].map((row) => (
                <div key={row.label} className="flex items-center justify-between rounded-md border border-border/70 px-2.5 py-2 text-sm">
                  <span>{row.label}</span>
      ...
      <div className="grid gap-3 lg:grid-cols-12">
        <Card className="border-border/70 bg-background lg:col-span-5">
          <Card.Content className="space-y-3 p-4">
            ...
            <Button size="sm" className="w-full">Continue with Email</Button>
            <Button size="sm" variant="outline" className="w-full">Continue with Google</Button>
          </Card.Content>
        </Card>
        <Card className="border-border/70 bg-background lg:col-span-7">
          <Card.Content className="space-y-3 p-4">
            ...
            <div className="rounded-lg border border-border/70 bg-muted/20 p-3">
              <p className="text-sm font-semibold">Two-factor authentication</p>
              <p className="text-xs text-muted-foreground">Recommended for all admin members.</p>
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              <div className="rounded-md border border-border/70 p-3 text-sm">Magic link</div>
              <div className="rounded-md border border-border/70 p-3 text-sm">Recovery codes</div>
            </div>
    ...
    <section className="border-t border-border/70 bg-background">
      <div className="mx-auto max-w-screen-xl px-4 py-10 md:px-6 md:py-12">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={() => window.dispatchEvent(new CustomEvent("almach-customizer-toggle"))}
...
function ExamplesSection() {
  const blocks = [
    ...
    },
    {
      ...
      preview: (
        <div className="space-y-2 rounded-lg border border-border/70 bg-background p-3">
          <div className="rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">john@example.com</div>
          <div className="rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">Password ••••••••</div>
          <div className="rounded-md bg-primary px-3 py-2 text-center text-xs font-semibold text-primary-foreground">Submit</div>
        </div>
      ),
    ...
    <section className="border-t border-border/70 bg-muted/10">
      <div className="mx-auto max-w-screen-xl px-4 py-10 md:px-6 md:py-12">
        <div className="mb-6 max-w-2xl">
          <h2 className="text-2xl font-semibold tracking-tight">Examples you can ship with</h2>
          <p className="mt-2 text-sm text-muted-foreground md:text-base">
            Reference snippets for components, forms, and server-state workflows.
          </p>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {blocks.map((block) => (
            <Card key={block.label} className="border-border/70 bg-card/50">
              <Card.Content className="space-y-3 p-4">
                <Badge variant="outline" className="w-fit font-mono text-[11px]">{block.label}</Badge>
                <div>
                ...
                <a href={block.href} className="inline-flex items-center gap-1.5 text-sm font-medium text-primary transition-opacity hover:opacity-80">
                  {block.cta}
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-14 md:px-6 md:py-16">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2.5">
              <a href="/getting-started" className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">
                Get started
              </a>
              <a href="/components" className="rounded-lg border border-border px-4 py-2 text-sm font-medium hover:bg-accent">
                View components
              </a>
            </div>

            <p className="mt-3 text-xs text-muted-foreground">React 18+ · Tailwind CSS v4 · Open source</p>
          </div>
